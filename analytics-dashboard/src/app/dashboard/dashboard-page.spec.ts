...
describe('DashboardPage', () => {
  ...
  it('should render the top pages with real data from the GA4 API', () => {
    expect(element.querySelector('app-top-pages h2')?.textContent).toContain(
      'Páginas más vistas',
    );

    const rows = element.querySelectorAll('app-top-pages .top-pages__row');
    expect(rows.length).toBe(2);
    expect(rows[0].querySelector('.top-pages__path')?.textContent).toContain('/inicio');
  ...
  it('should show an error message on top pages when the request fails', async () => {
    analyticsServiceStub.getTopPages.mockRejectedValueOnce(new Error('GA4 respondió 500'));
    ...
    expect(
      element.querySelector('app-top-pages .panel-card__status--error')?.textContent,
    ).toContain('No se pudieron cargar');
  });
